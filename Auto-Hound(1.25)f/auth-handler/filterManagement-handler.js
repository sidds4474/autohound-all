    ...
    if (resData.success) {
        ...
        keys.forEach((key, index) => {
            ...
            if (key === "yearMin") {
                ...
                tempYearMin = data
                updateSliders('year', tempYearMin, tempYearMax)

                // alert(data)
                // document.getElementById('year-slider-db').setAttribute('data-from', data)
            ...
            if (key === "yearMax") {
                ...
                // alert(data)
                // document.getElementById('year-slider-db').setAttribute('data-to', data)
                // userDataFilter.VEHICLE['Years'] = data
            ...
            if (key === "milesMax") {
                let data = valueKey.toString().trim()
                data = data.replace(',', '')
                    // alert(data)
                if (data == "200000+") {
                    data = "2005000"
                ...
                tempMilesMax = data
                updateSliders('miles', tempMilesMin, tempMilesMax)
            ...
            if (key === "milesMin") {
                ...
                // setLeftDisValue()
                // setLeftValueinit(inputLeftDis, inputRightDis, thumbLeftDis, thumbRightDis, rangeDis, 'dis')
                return
            }
...
const getDataAndBtn = (clsname, values) => {
    ...
    btns.forEach((btn) => {
        ...
        if (isPresentInd != -1) {
            console.log(isPresentInd, value, values);
...
const initializeBtns = (data) => {
    ...
    profitVal.innerText = "-"
    userData.VEHICLE['Profit (Minimum)'] = '$-'
    document.getElementById('no-notification').click()
    document.querySelector('html').style.display = "initial"
    return true
}
...
const createReqSmsGet = async(url, data) => {
    ...
    if (res_data.error) {
        ...
        smsSensitivity = res_data.sensitivityLevel
        let colorHexOfSensitivity = '#E73E3E';
        if(smsSensitivity == "GOOD!" || smsSensitivity == "GREAT!"){
            colorHexOfSensitivity = '#00b400';
        }
        document.getElementById('senstivityLevel').style.color = colorHexOfSensitivity
