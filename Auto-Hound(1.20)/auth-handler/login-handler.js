...
const createReqValidationLogin = (url, data) => {
    ...
    return new Promise(async(resolve, rej) => {
        ...
        if (res_data.tokenValidity) {
            resolve(res_data.incompleteAccountDetails)
        } else {
            rej('error')
        }
    })
...
const afterEndpointRes = async(res, username) => {
    console.log(res);
    if (res.access_token) {
        ...
        const resValidationIsPaymentIncomplete = await createReqValidationLogin(ValidationEndpoint, data)
        console.log(resValidationIsPaymentIncomplete);

        if (resValidationIsPaymentIncomplete) {
            window.location.href = "../account-management/account-page.html"
        } else {
            window.location.href = "../index.html?livelisting"
        }
