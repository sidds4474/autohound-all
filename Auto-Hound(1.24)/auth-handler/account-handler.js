...
const deleteAccountEndpoint = "http://68.183.105.196:8080/api/users/deleteUser?access_token="
...
const deleteUserReq = async(url) => {
    ...
    if (res_data.success && res_data.message === "User Deleted") {
...
const createReqAcc = async(url) => {
    ...
    if (!res_data.error) {
        ...
        if (res_data.incompleteAccountDetails) {
            incompleteAccount()
        }
...
const deleteUserAccount = () => {
    const token = getCookieAcc('access_token')
    deleteUserReq(deleteAccountEndpoint + token)
