...
const deleteAccount = () => {
    ...
    const head = "Are you sure you want to delete this account?"
    ...
    const leftbtnaction = 'deleteUserAccount()'
...
const incompleteAccount = () => {
    // removeprevlistener(modal)
    const head = "Incomplete Account!"
    const foot = " Before jumping in, please complete your Account page."
...
const insertDetails = (obj) => {
    ...
    var timesArr = [].slice.call(document.querySelectorAll('.minimal'))
    let timesArrIds = []
    timesArr.forEach((ele) => {
        timesArrIds.push(ele.id)
    })
    const k = Object.keys(obj)
    const vals = Object.values(obj)
    k.forEach((key, index) => {
        const i = timesArrIds.indexOf(key)
        if (i != -1) {
            document.getElementById(key).value = vals[index]
        }
    })
