const historyController = () => {
        ...
        if (!args) {
            history.pushState(null, null, '?livelisting')
        }
    }
    // historyController()
...
let colorindex = 0 //for auction page
let colorindexFilter = 0
const arrColor = ["#41a2f1", "#4e62b0", "#F76C6C", "#a1aff8", "#f4de6a", "#77A6F7", "#F4976C", "#41B3A3", "#97CAEF", "#97efbc"]
const filterColor = ["#C094F6", "#4287f5", "#4fdb7e", "#e8b45f", "#fc92f5", "#0266FF"] //for filter no longer used
let phoneLoadedCardsLastIndex = 0
const loadCardsPhone = () => {
    phoneLoadedCardsLastIndex = paginationPhone(phoneLoadedCardsLastIndex)
}
const filterColorSchema = {
    '1': '#41a2f1',
    '2': '#4e62b0',
    '3': '#F76C6C',
    '4': '#a1aff8',
...
const scrollList = (val) => {
    document.getElementsByClassName('list-container')[0].scrollBy(0, val)
}
var arrayList = []
var loadedPages = [1]
const loadMorePages = () => {
    ...
    if (loadedPages.length === pageArr.length) {
        document.getElementById('list-data-section-mobile').style.paddingBottom = "40px"
        document.getElementById('list-data-section').style.paddingBottom = "40px"
        document.querySelector('#list-section-footer').style.display = "none"
    } else {
        document.getElementById('list-data-section-mobile').style.paddingBottom = "25px"
        document.getElementById('list-data-section').style.paddingBottom = "25px"
    }

}
const loadMore = () => {
    const scrollyvalbefore = document.getElementsByClassName('list-container')[0].scrollTop
        // alert(scrollyvalbefore)
    loadCardsPhone()
    loadMorePages()
    const scrollyvalafter = document.getElementsByClassName('list-container')[0].scrollTop
        // alert(scrollyvalafter)
    const diff = scrollyvalafter - scrollyvalbefore
        // alert(diff)
    if (diff > 0) {
        document.getElementsByClassName('list-container')[0].scrollBy(0, -diff)
    }
}
...
const addingNewRowInList = (info) => {
    ...
    const filterArr = info.filtersNames //deprecated
    const filterColorHexArr = info.filterNameColorHexes
    let contentFilter = ""
    filterColorHexArr.forEach((filter) => {
        // let breakword = ""
        // let wordgr8tr10 = false
        // let wordCount = filter.name.split(' ')
        // wordCount.forEach((word) => {
        //     if (word.length >= 10) {
        //         console.log('word is here', word);
        //         wordgr8tr10 = true
        //     }
        // })
        // if (wordCount.length == 1 || wordgr8tr10) {
        //     breakword = "break-all"
        // }
        let contentBuff = ""
    ...
    colorindex = 0
    if (isSeeMore) {
        contentFilter += `<span class="seeMoreBtn" style="margin:2px;top:4px;position: relative;color: #0266ff; font-size:12px; margin-left:10px;cursor:pointer;">see more (${diffF})</span> `
    }
    ...
    // console.log(info.differencePrice)
    let isneg = info.differencePrice.split('$')[0]
    if (isneg === "-") {
        diffColor = "red"
    }
...
const addingNewFilter = (info) => {
    ...
    const content = Mustache.render($rowtemplatefilter, {
        isbreak: info.breakword,
        filterId: info.filterId,
        filterColor: info.color,
...
const toFilterPage = () => {
    if (numberOfFilters >= 10) {
        ...
        localStorage.setItem('lastActiveFilter', numberOfFilters + 1)
        window.location.href = "./filter-management-folders/filter-page.html"
    }
...
const deleteFilterModal = (id) => {
    ...
    const leftbtndata = '<span>YES, DELETE</span>'
...
const maxOf10 = () => {
    // removeprevlistener(modal)
    document.getElementById('filter-section-footer').style.display = "none"
    const head = "Filter limit exceeded. Please delete an existing filter to add a new one."
...
// const setFooter = (containerVal, footerVal) => {
//     const marginHeader = 45
//     const container = document.getElementsByClassName(containerVal)[0]
//     const footer = document.getElementById(footerVal)
//     footer.style.display = "flex"
//     footer.style.position = "relative"
//     const ch = container.offsetHeight
//     let footH = getOffsetTop(footer)
//     footH -= marginHeader
//     let diff = ch - footH
//     console.log('ch', ch);
